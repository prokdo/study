...
object Logic:
    ...
    /** Логическое ИЛИ */
    def or(a: Boolean, b: Boolean): Boolean = if a then true else b

    /** Логическое НЕ И */
    def nand(a: Boolean, b: Boolean): Boolean = if a then not(b) else true

    /** Логическое исключающее ИЛИ */
    def xor(a: Boolean, b: Boolean): Boolean = if a then not(b) else b

    /** Операция импликации */
    def impl(a: Boolean, b: Boolean): Boolean = if a then false else b
