object Lists:
    ...
    def nth[A](n: Int, ls: List[A]): A =
        if n < 0 then throw new NoSuchElementException
        else ls match
            case Nil => throw new NoSuchElementException
     ...
     * }}}
     */
    def rleEncode[A](ls: List[A]): List[(Int, A)] = pack(ls).map(x => (x.length, x.head))
     ...
     * }}}
     */
    def encodeModified[A](ls: List[A]): List[Any] = rleEncode(ls).map(x => if x._1 == 1 then x._2 else x)
     ...
     * }}}
     */
    def rleDecode[A](ls: List[(Int, A)]): List[A] = ls.flatMap(x => List.fill(x._1)(x._2))
     ...
     * }}}
     */
    def dropN[A](n: Int, ls: List[A]): List[A] = ls.zipWithIndex.filter(x => (x._2 + 1) % n != 0).map(_._1)
     ...
     */
    def slice[A](start: Int, end: Int, ls: List[A]): List[A] = ls.drop(start).take(end - start)

    /** Формирует список, содержащий все целые числа в заданном диапазоне
     *
     * (!) В ходе реализации нельзя использовать стандартные методы списка для формирования интервалов
     */
    def range(start: Int, end: Int): List[Int] = if start > end then Nil else start :: range(start + 1, end)
