...
public final class Solver {
    ...
    public static Knapsack[][] solve(int capacity, ArrayList<Item> items) {
                ...
                if (i == 0 || j == 0) {
                    ...
                    if (items.get(i - 1).getWeight() > j) {
                        ...
                        int newPrice = items.get(i - 1).getPrice()
                                + matrix[i - 1][j - items.get(i - 1).getWeight()].getCurrentPrice();

                        if (matrix[i - 1][j].getCurrentPrice() > newPrice) {
                            matrix[i][j] = matrix[i - 1][j];
                        } else {
                            matrix[i][j] = new Knapsack(capacity);

                            for (Item item : matrix[i - 1][j - items.get(i - 1).getWeight()].getItems()) {
