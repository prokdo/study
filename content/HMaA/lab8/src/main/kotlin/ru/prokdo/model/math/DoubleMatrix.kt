...
class DoubleMatrix : Iterable<DoubleArray> {
    ...
    companion object {
        const val POSITIVE_INFINITY = Double.MAX_VALUE
        const val NEGATIVE_INFINITY = Double.MIN_VALUE
    ...
    fun toString(precision: Int): String {
        ...
        this.forEachIndexed { rowIndex, row -> run {
            row.forEachIndexed { columnIndex, value -> run {
                if (value == DoubleMatrix.POSITIVE_INFINITY || value == DoubleMatrix.NEGATIVE_INFINITY) builder.append("INF")
                else builder.append(".${precision}f".format(value))
