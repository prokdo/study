...
def encode(to_encode: BinaryStr) -> BinaryStr:
    ...
    logging.info(
        ...
        f'{"-\t" * (len(data))}\n'
        f'{"\n".join(
            " \t".join(str(el) for el in row) + f" \tr{2**i} = [{data[2 ** i][0]}]"
            for i, row in enumerate(transformation_matrix)
        )}\n'
    )
...
def decode(to_decode: BinaryStr):
    """
    ...
    while True:
        if is_corrected:
            ...
            logging.info(f'Была исправлена одна ошибка, обнаружено наличие дальнейших ошибок')
            break
        ...
        logging.info(
            ...
            f'{"-\t" * (len(temp_data))}\n'
            f'{"\n".join(
                " \t".join(str(el) for el in row) + f" \tr{2**i} = [{temp_data[2 ** i][0]}]"
                for i, row in enumerate(transformation_matrix)
            )}'
        )
        if not doesnt_match:
